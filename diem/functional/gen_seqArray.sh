...
do
    ...
    then
        ...
        if [ $ret -eq 0 ]; then
            ...
            timeout 10 cargo run --release --quiet --package move-prover -- -d ~/diem/language/move-stdlib/modules $move_dir/$filename.move --use-cvc4 --vector-theory SmtSeq -v debug -k > $bm_dir_single/mvp_seqArray.log 2>&1
            rm *.smt
            rm *.bpl.log
            ...
            bm_bpl=$bm_dir_single/$filename.seqArray.bpl
            ...
            if [ -f "$bm_smt2" ]; then
                ...
                timeout 20 $cvc5 $bm_dir_single/$bm_smt2 --incremental --strings-exp --strings-seq-update > $bm_dir_single/seqArray.output 2>&1
            fi
        fi
    fi
done
