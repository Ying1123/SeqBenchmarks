...
do
    ...
    then
        ...
        if [ $ret -eq 0 ]; then
            ...
            cargo run --release --quiet --package move-prover -- -d ~/diem/language/move-stdlib/modules $move_dir/$filename.move --use-cvc4 --vector-theory SmtSeq --generate-smt -v debug -k > $bm_dir_single/mvp_cvc5.log 2>&1
            rm *.smt
            rm *.bpl.log
